...
# __file__ = app/backend/core/template_engine.py
# 따라서 4단계 위로 올라가야 프로젝트 루트(law-crawler-ui)에 도달
BASE_DIR = os.path.dirname(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
)
template_dir = os.path.join(BASE_DIR, "app", "frontend", "views")
logger.debug("------------------------------------------------")
...
def get_template_html(template_name):
    ...
    if template_str.find("<body") != -1:
        body_content = re.search(
            r"<body[^>]*>(.*?)</body>", template_str, re.DOTALL
        ).group(1)
    else:
